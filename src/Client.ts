...
export class Client {
  ...
  async onmessage(event: MessageEvent) {
    ...
    if (message.topic && message.command == MessageCommand.PUBLISH) {
      ...
      this.callbacks['suback'].map(value => value({
        topic: message.topic
      }));
    } else if (message.topic && message.command == MessageCommand.UNSUBACK) {
      this.callbacks['unsuback'].map(value => value({
        topic: message.topic
      }));
    }
  }
  ...
  removeListener(event: ClientEvents, callback: ClientEventCallback) {
    this.callbacks[event] = this.callbacks[event].filter(_callback => _callback !== callback)
  }

  async wait(event: ClientEvents, check: (context:ClientEventContext) => boolean = () => true) {
